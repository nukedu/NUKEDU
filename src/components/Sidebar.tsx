...
export default function Sidebar({ isOpen, onClose, activeMenu, onMenuChange }: SidebarProps) {
  ...
  const userInitials = profile?.avatar || userName.split(' ').slice(0, 2).map(n => n[0]).join('').toUpperCase();
  const userRole = profile?.role === 'super-admin' ? 'Super Admin' :
                   profile?.role === 'admin-mwc' ? 'Admin MWC' :
                   profile?.role === 'ketua-ranting' ? 'Ketua Ranting' :
                   profile?.role === 'sekretaris' ? 'Sekretaris' :
                   profile?.role === 'bendahara' ? 'Bendahara' :
                   profile?.role === 'kontributor' ? 'Kontributor Aktif' : 'Anggota';
  const userPoints = profile?.points || 0;
  const levelName = userPoints >= 1000 ? 'Platinum' : userPoints >= 700 ? 'Gold' : userPoints >= 400 ? 'Silver' : 'Bronze';
  const levelEmoji = userPoints >= 1000 ? '💎' : userPoints >= 700 ? '⭐' : userPoints >= 400 ? '🥈' : '🥉';
  const levelPercent = Math.min(100, (userPoints / 1000) * 100);
  ...
  return (
    ...
    >
      ...
      <div className="px-6 py-4 border-b border-emerald-700/50">
        <div className="flex items-center gap-3">
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex items-center gap-1.5">
              ...
              <span className="text-emerald-300 text-xs">{userRole}</span>
            </div>
          </div>
        </div>
        <div className="mt-3 bg-emerald-700/40 rounded-lg p-2.5">
          <div className="flex items-center justify-between text-xs mb-1.5">
            ...
            <span className="font-bold text-amber-300">{levelName} {levelEmoji}</span>
          </div>
          <div className="w-full bg-emerald-900/60 rounded-full h-1.5">
            <div className="bg-gradient-to-r from-amber-400 to-yellow-300 h-1.5 rounded-full" style={{ width: `${levelPercent}%` }} />
          </div>
          <p className="text-[10px] text-emerald-300 mt-1">{userPoints} / 1000 poin menuju {userPoints >= 1000 ? 'Diamond' : 'Platinum'}</p>
        </div>
      </div>
      ...
      <div className="p-4 border-t border-emerald-700/50">
        <button onClick={logout} className="w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm text-emerald-300 hover:bg-red-500/20 hover:text-red-300 transition-all">
